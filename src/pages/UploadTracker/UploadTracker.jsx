...
//   Alert, AlertTitle, LinearProgress, List, ListItem, ListItemIcon,
//   ListItemText, Divider, Link, IconButton, Tooltip
// } from '@mui/material';
...
//   if (!jobStatus) return <CircularProgress size={20} />;
...
//                 <Button variant="outlined" size="small" color="warning" startIcon={<DownloadIcon />} href={`${serviceUrl}${jobStatus.error_csv_download_url}`} sx={{ mt: 2 }}>
...
//       }
//     });
//     return () => { Object.values(pollingIntervals).forEach(clearInterval); };
//   }, [trackedJobs, fetchJobStatus]);

//   const handleManualTrack = () => {
//     const idToTrack = manualId.trim();
//     if (idToTrack) {
//       if (!recentUploads.includes(idToTrack)) {
//         const newRecent = [idToTrack, ...recentUploads].slice(0, 5);
//         setRecentUploads(newRecent);
//         localStorage.setItem('recentUploads', JSON.stringify(newRecent));
//       }
//       fetchJobStatus(idToTrack);
//       setManualId('');
//     }
//   };

//   const refreshAll = () => {
//     setTrackedJobs({}); // Clear old data to force re-fetch appearance
//     recentUploads.forEach(id => fetchJobStatus(id));
//   };
...
//         <Typography variant="h6" gutterBottom>Track a New Upload by ID</Typography>
//         <Grid container spacing={2} alignItems="center">
//           <Grid item xs={12} sm={9}><TextField fullWidth label="Enter Tracking ID (upload_id)" value={manualId} onChange={(e) => setManualId(e.target.value)} size="small" /></Grid>
//           <Grid item xs={12} sm={3}><Button fullWidth variant="contained" onClick={handleManualTrack} startIcon={<TravelExploreIcon />} disabled={!manualId.trim()}>Track</Button></Grid>
//         </Grid>
//       </Paper>
